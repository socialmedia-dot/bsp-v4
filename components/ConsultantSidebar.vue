    ...
    <aside class="sidebar" :class="{ open: isOpen }">
      <nav class="sidebar-nav">
        <div class="nav-section">
          <div class="nav-section-title">Consultant Portal</div>
          <NuxtLink to="/consultant/dashboard" :class="['nav-item', { active: active === 'dashboard' }]" @click="close">
            <span class="nav-icon">📊</span><span>Dashboard</span>
          </NuxtLink>
          <NuxtLink to="/consultant/applications" :class="['nav-item', { active: active === 'applications' }]" @click="close">
            <span class="nav-icon">📋</span><span>Applications</span>
          </NuxtLink>
          <NuxtLink to="/consultant/profile" :class="['nav-item', { active: active === 'profile' }]" @click="close">
            <span class="nav-icon">💼</span><span>My Profile</span>
          </NuxtLink>
          <NuxtLink to="/consultant/partnerships" :class="['nav-item', { active: active === 'partnerships' }]" @click="close">
            <span class="nav-icon">🤝</span><span>Partnerships</span>
          </NuxtLink>
          <NuxtLink to="/consultant/reports" :class="['nav-item', { active: active === 'reports' }]" @click="close">
...
.sidebar { background: #fff; border-right: 1px solid #e2e8f0; flex-shrink: 0; height: 100vh; overflow-y: auto; position: sticky; top: 0; width: 260px; z-index: 50; }
